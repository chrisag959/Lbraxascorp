...
|  --- | --- | --- |
|  serverUri | string | <i>(Optional)</i> The server websocket uri to connect to. |
|  userText | string | <i>(Optional)</i> A custom text string that is sent to the orchestration server. |
|  accessToken | string | <i>(Optional)</i> A jwt access token issued to permit access to this server. |
|  retryOptions | [RetryOptions](./smwebsdk.retryoptions.md) | <i>(Optional)</i> Options for customizing connection error retry. |
